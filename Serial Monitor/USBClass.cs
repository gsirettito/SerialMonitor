...
namespace USBClassLibrary {
    ...
    public class USBClass {
        ...
        public class Win32Wrapper {
            ...
            public enum DEVICE_SERVICE : uint {
                Any = 0x00000000,
                USBSTOR = 0x00000001,
                DISK = 0x00000002,
                WUDFWPDFS = 0x00000003,
                VOLUME = 0x00000004,
                CDROM = 0x00000005,
                USBVIDEO = 0x00000006,
                USBCCGP = 0x00000007,
                WUDFWPDMTP = 0x00000008,
                MODEM = 0x00000009,
                WINUSB = 0x0000000A,
                HIDUSB = 0x0000000B,
                USBAUDIO = 0x0000000C,
        ...
        public class USBDeviceEventArgs : EventArgs {
            ...
            public USBDeviceEventArgs(Guid guid, string hardwareID) {
                DeviceClass = guid;
        ...
        public void ProcessWindowsMessage(WindowMessage Msg, IntPtr WParam, IntPtr LParam, ref bool handled) {
                ...
                switch (WParam.ToInt32()) {
                    case (int)Win32Wrapper.DBTDEVICE.DBT_DEVICEARRIVAL:
                        ...
                        if (devType == Win32Wrapper.DBTDEVTYP.DBT_DEVTYP_DEVICEINTERFACE) {
                            ...
                            int size = Marshal.SizeOf(deviceInterface);
                            deviceInterface.dbcc_size = size;
                            deviceInterface = (Win32Wrapper.DEV_BROADCAST_DEVICEINTERFACE)Marshal.PtrToStructure(LParam, typeof(Win32Wrapper.DEV_BROADCAST_DEVICEINTERFACE));
                            var guid = deviceInterface.dbcc_classguid;
                            var hID = deviceInterface.dbcc_name;
                            USBDeviceEventArgs e = new USBDeviceEventArgs(guid, hID);
        ...
        public class DeviceProperties {
            ...
            public USBClass.Win32Wrapper.DEVICE_SERVICE Service { get; set; }
            public Guid ClassGuid { get; set; }
            public Guid ContainerID { get; set; }
            public string HardwareID { get; set; }
            public string ParentIdPrefix { get; set; }
            public DeviceProperties Parent { get; set; }
        }
                    ...
                    while (Success) {
                        if (Success) {
                            ...
                            if (Success) {
                                ...
                                DP.ClassGuid = DevInfoData.ClassGuid;
                                ...
                                if (LastError == Win32Wrapper.WinErrors.ERROR_INSUFFICIENT_BUFFER) {
                                    ...
                                    if (RequiredSize <= BUFFER_SIZE) {
                                        if (Win32Wrapper.SetupDiGetDeviceRegistryProperty(h, ref DevInfoData, (UInt32)Win32Wrapper.SPDRP.SPDRP_HARDWAREID, ref RegType, IntPtrBuffer, BUFFER_SIZE, ref RequiredSize)) {
                                            DP.HardwareID = Marshal.PtrToStringAuto(IntPtrBuffer);
                                            DP.HardwareID = DP.HardwareID.ToLowerInvariant();
                                            if (
                                                DP.HardwareID.Contains(ExpectedDeviceID) &&
                                                    (
                                                        ((MI != null) && DP.HardwareID.Contains(ExpectedInterfaceID))
                                                        ||
                                                        ((MI == null) && (!DP.HardwareID.Contains("mi")))
                                                    )
                                                ) {
                                                //Status = true; //Found device

                                                DP.ContainerID = Guid.Empty;
                                                if (Win32Wrapper.SetupDiGetDeviceRegistryProperty(h, ref DevInfoData, (UInt32)Win32Wrapper.SPDRP.SPDRP_BASE_CONTAINERID, ref RegType, IntPtrBuffer, BUFFER_SIZE, ref RequiredSize))
                                                    DP.ContainerID = new Guid(Marshal.PtrToStringAuto(IntPtrBuffer));

                                                DP.Service = Win32Wrapper.DEVICE_SERVICE.Any;
                                                if (Win32Wrapper.SetupDiGetDeviceRegistryProperty(h, ref DevInfoData, (UInt32)Win32Wrapper.SPDRP.SPDRP_SERVICE, ref RegType, IntPtrBuffer, BUFFER_SIZE, ref RequiredSize))
                                                    DP.Service = (Win32Wrapper.DEVICE_SERVICE)Enum.Parse(typeof(Win32Wrapper.DEVICE_SERVICE), Marshal.PtrToStringAuto(IntPtrBuffer).ToUpper());
        ...
        public static bool GetUSBDevices(ref List<DeviceProperties> ListOfDP) {
            return GetUSBDevices(ref ListOfDP, Guid.Empty);
        }

        public static bool GetUSBDevices(ref List<DeviceProperties> ListOfDP, string DevEnum) {
            return GetUSBDevices(ref ListOfDP, Guid.Empty, DevEnum);
        }

        public static bool GetUSBDevices(
            ref List<DeviceProperties> ListOfDP,
            Guid classGuid,
            string DevEnum = "USB",
            Win32Wrapper.DEVICE_SERVICE service = Win32Wrapper.DEVICE_SERVICE.Any) {
            IntPtr IntPtrBuffer = Marshal.AllocHGlobal(BUFFER_SIZE);
            IntPtr h = IntPtr.Zero;
            Win32Wrapper.WinErrors LastError;
            //bool Status = false;

            ListOfDP = new List<DeviceProperties>();
            try {
                h = Win32Wrapper.SetupDiGetClassDevs(IntPtr.Zero, DevEnum, IntPtr.Zero, (int)(Win32Wrapper.DIGCF.DIGCF_PRESENT | Win32Wrapper.DIGCF.DIGCF_ALLCLASSES));
                    ...
                    while (Success) {
                        if (Success) {
                            ...
                            if (Success) {
                                ...
                                if (classGuid != Guid.Empty && DevInfoData.ClassGuid != classGuid) {
                                    i++;
                                    continue;
                                }
                                ...
                                if (LastError == Win32Wrapper.WinErrors.ERROR_INSUFFICIENT_BUFFER) {
                                    if (RequiredSize <= BUFFER_SIZE) {
                                        if (Win32Wrapper.SetupDiGetDeviceRegistryProperty(h, ref DevInfoData, (UInt32)Win32Wrapper.SPDRP.SPDRP_HARDWAREID, ref RegType, IntPtrBuffer, BUFFER_SIZE, ref RequiredSize)) {
                                            ...
                                            DP.Service = Win32Wrapper.DEVICE_SERVICE.Any;
                                            string _service = Enum.GetName(typeof(Win32Wrapper.DEVICE_SERVICE), service);
                                            if (Win32Wrapper.SetupDiGetDeviceRegistryProperty(h, ref DevInfoData, (UInt32)Win32Wrapper.SPDRP.SPDRP_SERVICE, ref RegType, IntPtrBuffer, BUFFER_SIZE, ref RequiredSize)) {
                                                try { DP.Service = (Win32Wrapper.DEVICE_SERVICE)Enum.Parse(typeof(Win32Wrapper.DEVICE_SERVICE), Marshal.PtrToStringAuto(IntPtrBuffer).ToUpper()); } catch { };
                                                if (service != Win32Wrapper.DEVICE_SERVICE.Any && DP.Service != service) {
                                                    i++;
                                                    continue;
                                                }
                                            }
        ...
        public static bool GetDevices(
            ...
            ListOfDP = new List<DeviceProperties>();
            try {
                h = Win32Wrapper.SetupDiGetClassDevs(IntPtr.Zero, "Ports", IntPtr.Zero, (int)(Win32Wrapper.DIGCF.DIGCF_PRESENT | Win32Wrapper.DIGCF.DIGCF_ALLCLASSES));
                if (h.ToInt64() != INVALID_HANDLE_VALUE) {
                    bool Success = true;
                    uint i = 0;
                    while (Success) {
        ...
        public static bool GetDeviceByClass(ref List<DeviceProperties> ListOfDP, Guid classGuid) {
            IntPtr IntPtrBuffer = Marshal.AllocHGlobal(BUFFER_SIZE);
            IntPtr h = IntPtr.Zero;
            Win32Wrapper.WinErrors LastError;
            bool Status = false;
            ...
            ListOfDP = new List<DeviceProperties>();
            try {
                h = Win32Wrapper.SetupDiGetClassDevs(ref classGuid, IntPtr.Zero, IntPtr.Zero, (uint)(Win32Wrapper.DIGCF.DIGCF_PRESENT | Win32Wrapper.DIGCF.DIGCF_DEVICEINTERFACE));
                if (h.ToInt64() != INVALID_HANDLE_VALUE) {
                    bool Success = true;
                    uint i = 0;
                    while (Success) {
                        if (Success) {
                            ...
                            DeviceInterfaceData.cbSize = (uint)Marshal.SizeOf(DeviceInterfaceData);
                            //Start the enumeration
                            Success = Win32Wrapper.SetupDiEnumDeviceInterfaces(h, IntPtr.Zero, ref classGuid, i, ref DeviceInterfaceData);
                            if (!Success) {
                                LastError = (Win32Wrapper.WinErrors)Marshal.GetLastWin32Error();
                                if (LastError == Win32Wrapper.WinErrors.ERROR_NO_MORE_ITEMS) {
                                    Status = false;
                            ...
                            Success = Win32Wrapper.SetupDiGetDeviceInterfaceDetail(h, ref DeviceInterfaceData, ref DeviceInterfaceDetailData, BUFFER_SIZE, out RequiredSize, ref DevInfoData);
